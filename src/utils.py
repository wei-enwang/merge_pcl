...
import numpy as np
from transformations import quaternion_from_matrix, unit_vector, quaternion_multiply, random_quaternion
...
def rotate_quat(body, quat):
    set_pose(body, (get_point(body), quaternion_multiply(get_quat(body), quat)))
...
def dump_body_yd(body):
    ...
    body_info['orientation'] = p.getEulerFromQuaternion(np.array(pos_info[1]))
...
def in_collision():
    ...
    for i in range(len(bodies)):
        for j in range(0, i):
            collision = pairwise_collision(bodies[i], bodies[j])
            if collision:
                return True
...
def random_shuffle_on_aabb(top_body, bottom_aabb, top_pose=unit_pose(),
                             percent=1.0, max_attempts=50, epsilon=1e-3):
    """
    Randomly reset the position and orientation of an object on top of the "floor".
    """
    for _ in range(max_attempts):
        theta1 = np.random.uniform(*CIRCULAR_LIMITS)
        theta2 = np.random.uniform(*CIRCULAR_LIMITS)
        theta3 = np.random.uniform(*CIRCULAR_LIMITS)
        rotation = Euler(yaw=theta1, pitch=theta2, roll=theta3)
        set_pose(top_body, multiply(Pose(euler=rotation), top_pose))
        center, extent = get_center_extent(top_body)
        lower = (np.array(bottom_aabb[0]) + percent*extent/2)[:2]
        upper = (np.array(bottom_aabb[1]) - percent*extent/2)[:2]
...
def random_placement(top_body, bottom_body, bottom_link=None, **kwargs):
    bottom_aabb = get_aabb(bottom_body, link=bottom_link)
    return random_shuffle_on_aabb(top_body, bottom_aabb, **kwargs)
